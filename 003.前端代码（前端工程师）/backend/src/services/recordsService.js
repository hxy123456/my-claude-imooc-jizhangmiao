...
/**
 * 将 MySQL DATE/DATETIME 列值转为 'YYYY-MM-DD' 字符串。
 * 不用 toISOString() —— 当 mysql2 dateStrings:false 时，DATE 列会被转成
 * 服务器本地时区的 Date(YYYY-MM-DD 00:00:00 +08:00)，
 * .toISOString() 会按 UTC 回退一天（中国时区 UTC+8）。
 */
...
function toRecord(row) {
  ...
  return {
    ...
    note: row.note,
    recordDate: _dateToStr(row.record_date),
